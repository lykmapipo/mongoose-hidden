...
describe("mongoose-hidden", function() {
  ...
  let testUserSub = { name: "Joe", email: "joe@example.com", password: "secret",
    spouse: { name: "Maries" } }
  let testUserSub2 = { name: "Joe", email: "joe@example.com", password: "secret",
    spouse: { name: "Maries", age: 37 } }
  let testUser2 = { name: "Marie", email: "marie@example.com", password: "secret" }
  let testUser3 = { name: "Joe", email: { prefix: 'joe', suffix: 'example.com' },
    password: "secret" }
  ...
  let defineModel = function(name, schemaProperties, pluginOptions) {
    ...
    let schema = schemaProperties instanceof Schema ? schemaProperties :
      new Schema(schemaProperties)
  ...
  describe("A model with a virtuals defined", function() {
    ...
    it("should return and hide nested virtuals", function(done) {
      ...
      let User = mongoose.model('VirtualUser', schema)
      let user = new User(Object.assign({ nice: { bro: 'foo' } },
        testUser))
  ...
  describe("A documents with non-schema properties set to hidden", function() {
    it("should hide the properties", function(done) {
      ...
      user.save(function(err, savedUser) {
        ...
        let User2 = mongoose.model('User', User.schema,
  ...
  describe("Model with Single Embedded Subdoc", function() {
    ...
    it("shouldn't return those property", function(done) {
      var User = defineModel({
        ...
        email: String,
        password: String,
        spouse: new Schema({ name: { type: String, hide: true },
          age: { type: Number } })
      });
